...
import { Grid, Box, Card, Button, Alert, TextField, Typography, TableContainer, TableHead, TableRow, TableCell, TableBody, Table } from "@mui/material";
import { store } from 'host/store';
import { useSelector, useDispatch } from 'react-redux';

export default function () {
  const countryName = 'India'; // Example country name, can be dynamic
  ...
  return (
    <Grid xs={12} sm={4} md={4} lg={4} xl={4} item>
      <Card sx={{ backgroundColor: '#f5f5f5', padding: '10px' }}>
        <Alert severity="info">{'MFE 2 getting data from Host and On Submit sharing data to MFE 3 via redux'}</Alert>
        <Box>
          <TextField
            label="Enter Name"
            variant="outlined"
